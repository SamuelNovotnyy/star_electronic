  ...
  if (loading) {
    return (
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="skeleton h-64 w-full rounded-xl break-inside-avoid" />
        ))}
      </div>
    );
  }
    ...
    <>
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
        {items.map((it) => (
          <div
            key={it.id}
            className="group relative break-inside-avoid rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer"
            ...
            */}
            {it.description && (
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <p className="text-white text-sm font-medium line-clamp-2">{it.description}</p>
              </div>
            )}
          </div>
        ))}
          ...
          >
            ...
            {selectedImage.description && (
              <div className="mt-6 bg-black/50 backdrop-blur-md rounded-xl px-8 py-4 text-white text-center max-w-3xl shadow-lg border border-white/10 shrink-0">
                <p className="text-lg font-medium tracking-wide">{selectedImage.description}</p>
              </div>
            )}
          </div>
        </div>
